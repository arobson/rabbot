...
describe( 'without valid connection criteria', function() {
	it( 'should throw a meaninful error', function( done ) {
		rabbit.on( 'silly.connection.failed', function( data ) {
			data.err.should.have.property( 'message', 'getaddrinfo ENOTFOUND' );
			data.name.should.equal( 'silly' );
...
describe( 'with disconnect', function() {
	...
	it( 'should mark connection as closed (not open)', function() {
		rabbit.connections[ 'default' ].isOpen.should.be.false;
	} );

	describe( 'with a valid exchange', function() {
		before( function( done ) {
			rabbit.addExchange( 'pendingExchange', 'fanout', {
				autoDelete: true
			}, 'default' )
				.then( function() {
		...
		it( 'should be possible to add a pending message', function( done ) {
			var message = rabbit.addPendingMessage( 'pendingExchange', 'pending.message', 'I\'m waiting!' );
			seqNo = rabbit._sequenceNo;
			rabbit.should.have.property( 'pendingMessages' );
			rabbit.pendingMessages[ seqNo ].should.have.properties( {
				'message': 'I\'m waiting!',
				'sequenceNo': seqNo
			} );
			done();
